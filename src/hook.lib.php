...
class GddysecHook extends GddysecEvent
{
    ...
    public static function hookLoginFailure($title = '')
    {
        ...
        $logins = gddysec_get_failed_logins();

        if (is_array($logins) && !empty($logins)) {
            $max_time = 3600; /* report logins in the last hour */
            $maximum = GddysecOption::getOption(':maximum_failed_logins');
            ...
            if ($logins['diff_time'] <= $max_time && $logins['count'] >= $maximum) {
                 ...
                 */
                gddysec_report_failed_logins($logins);
    ...
    public static function hookOptionsManagement()
    {
        ...
        if (current_user_can('manage_options') && GddysecOption::checkOptionsNonce()) {
            /* compare settings in the database with the modified ones */
            $options_changed = GddysecOption::whatOptionsWereChanged($_POST);
            $options_changed_str = '';
    ...
    public static function hookPluginUpdate()
    {
        ...
        if (GddysecRequest::getOrPost('action', $plugin_update_actions)
            || GddysecRequest::getOrPost('action2', $plugin_update_actions)
        ) {
            $plugin_list = array();
            $items_affected = array();
    ...
    public static function hookPostBeforeDelete($id = 0)
    {
        ...
        $cache = new GddysecCache('hookdata');
    ...
    public static function hookPostStatus($new = '', $old = '', $post = null)
    {
        ...
        if (GddysecOption::isIgnoredEvent($post_type)) {
        ...
        if (GddysecOption::isIgnoredEvent($custom_type)) {
    ...
    public static function hookThemeDelete()
    {
        ...
        if (current_user_can('delete_themes')
            && GddysecRequest::getOrPost('action', 'delete')
            && GddysecRequest::getOrPost('stylesheet', '.+')
        ) {
            $theme = GddysecRequest::getOrPost('stylesheet', '.+');
