 ...
 * @link       https://wordpress.org/plugins/godaddy-security
 */
class GddysecMail extends GddysecOption
{
    /**
    ...
    public static function sendMail($email = '', $subject = '', $message = '', $data_set = array())
    {
         ...
         */
        if (GddysecOption::isEnabled(':use_wpmail')) {
    ...
    private static function prettifyMail($subject = '', $message = '', $data_set = array())
    {
        ...
        if (strpos($message, 'multiple') !== false) {
            $message_parts = GddysecAPI::parseMultipleEntries($message);
        ...
        $params['TemplateTitle'] = 'GoDaddy Security Alert';
        ...
        $params['Time'] = Gddysec::datetime();

        foreach ($data_set as $var_key => $var_value) {
            $params[ $var_key ] = $var_value;
        }

        /* GddysecTemplate::notification-pretty */
        /* GddysecTemplate::notification-simple */
        return GddysecTemplate::getSection('notification-' . $prettify_type, $params);
    }
