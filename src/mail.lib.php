...
class GddysecMail extends GddysecOption
{
    ...
    public static function send_mail($email = '', $subject = '', $message = '', $data_set = array())
    {
        ...
        if (!self::emails_per_hour_reached() || $force || $debug) {
             ...
             * @var boolean
             */
            if (GddysecOption::is_enabled(':use_wpmail')) {
    ...
    private static function get_email_subject($event = '')
    {
        ...
        $subject = str_replace(':remoteaddr', self::remoteAddr(), $subject);
    ...
    private static function prettify_mail($subject = '', $message = '', $data_set = array())
    {
        ...
        $mail_variables = array(
            'TemplateTitle' => 'GoDaddy Security Alert',
            'Subject' => $subject,
            'Website' => self::get_option('siteurl'),
            'RemoteAddress' => self::remoteAddr(),
            'Message' => $message,
            'User' => $display_name,
            'Time' => Gddysec::current_datetime(),
        );
        ...
        return GddysecTemplate::getSection($template_name, $mail_variables);
    }
