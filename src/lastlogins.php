 ...
 * @link       https://wordpress.org/plugins/godaddy-security
 */
class GddysecLastLogins extends Gddysec
{
}
 ...
 * @return void
 */
function gddysec_lastlogins_admins()
{
    ...
    foreach ((array) $admins as $admin) {
        $last_logins = gddysec_get_logins(5, 0, $admin->ID);
        $user_snippet = array(
            ...
            'AdminUsers.UserURL' => Gddysec::adminURL('user-edit.php?user_id=' . $admin->ID),
        ...
        if ($last_logins
            ...
            foreach ($last_logins['entries'] as $i => $lastlogin) {
                if ($i === 0) {
                    $user_snippet['AdminUsers.RegisteredAt'] = Gddysec::datetime(
                        $lastlogin['user_registered_timestamp']
                    );
                }

                $user_snippet['AdminUsers.LastLogins'] .= GddysecTemplate::getSnippet(
                    'lastlogins-admins-lastlogin',
                    array(
                        'AdminUsers.RemoteAddr' => $lastlogin['user_remoteaddr'],
                        'AdminUsers.Datetime' => Gddysec::datetime($lastlogin['user_lastlogin_timestamp']),
                    )
                );
            }
        }

        $params['AdminUsers.List'] .= GddysecTemplate::getSnippet('lastlogins-admins', $user_snippet);
    }

    return GddysecTemplate::getSection('lastlogins-admins', $params);
}
 ...
 */
function gddysec_lastlogins_all()
{
    $max_per_page = GDDYSEC_LASTLOGINS_USERSLIMIT;
    ...
    if (!gddysec_lastlogins_datastore_is_writable()) {
        $fpath = Gddysec::escape(gddysec_lastlogins_datastore_filepath());
        GddysecInterface::error(sprintf('Last-logins data file is not writable: <code>%s</code>', $fpath));
    }

    $last_logins = gddysec_get_logins($max_per_page, $offset);
    ...
    if (is_array($last_logins) && !empty($last_logins)) {
        ...
        foreach ($last_logins['entries'] as $user) {
            $user_dataset = array(
                ...
                'UserList.TimeAgo' => Gddysec::humanTime($user['user_lastlogin_timestamp']),
                'UserList.UserURL' => Gddysec::adminURL('user-edit.php?user_id=' . $user['user_id']),
            );
            ...
            $params['UserList'] .= GddysecTemplate::getSnippet('lastlogins-all', $user_dataset);
        }

        // Generate the pagination for the list.
        $params['UserList.Pagination'] = GddysecTemplate::pagination(
            '%%GDDYSEC.URL.Lastlogins%%',
            $last_logins['total'],
            $max_per_page
        );
    }

    return GddysecTemplate::getSection('lastlogins-all', $params);
}
 ...
 */
function gddysec_lastlogins_datastore_filepath()
{
    return Gddysec::dataStorePath('gddysec-lastlogins.php');
}
 ...
 */
function gddysec_lastlogins_datastore_exists()
{
    $fpath = gddysec_lastlogins_datastore_filepath();
 ...
 */
function gddysec_lastlogins_datastore_is_writable()
{
    $datastore_filepath = gddysec_lastlogins_datastore_exists();
 ...
 */
function gddysec_lastlogins_datastore_is_readable()
{
    $datastore_filepath = gddysec_lastlogins_datastore_exists();
...
if (!function_exists('gddysec_set_lastlogin')) {
    /**
     ...
     * @return void
     */
    function gddysec_set_lastlogin($user_login = '')
    {
        $filename = gddysec_lastlogins_datastore_is_writable();
    ...
    add_action('wp_login', 'gddysec_set_lastlogin', 50);
}
 ...
 */
function gddysec_get_logins($limit = 10, $offset = 0, $user_id = 0)
{
    $limit = intval($limit); /* prevent arbitrary user input */
    $datastore_filepath = gddysec_lastlogins_datastore_is_readable();
    $last_logins = array(
        'total' => 0,
        'entries' => array(),
    );
    ...
    if (!$datastore_filepath) {
        return Gddysec::throwException('Invalid last-logins storage file');
    }

    $parsed_lines = 0;
    $data_lines = GddysecFileInfo::fileLines($datastore_filepath);

    if (!$data_lines) {
        return Gddysec::throwException('No last-logins data is available');
    }
...
if (!function_exists('gddysec_login_redirect')) {
    /**
     ...
     */
    function gddysec_login_redirect($redirect_to = '', $request = null, $user = false)
    {
        $login_url = !empty($redirect_to) ? $redirect_to : Gddysec::adminURL();

        if ($user instanceof WP_User
            && in_array('administrator', $user->roles)
            && GddysecOption::isEnabled(':lastlogin_redirection')
        ) {
            $login_url = add_query_arg('gddysec_lastlogin', 1, $login_url);
        }

        return $login_url;
    }

    if (GddysecOption::isEnabled(':lastlogin_redirection')) {
        add_filter('login_redirect', 'gddysec_login_redirect', 10, 3);
    }
}

if (!function_exists('gddysec_get_user_lastlogin')) {
    /**
     * Display the last user login at the top of the admin interface.
     *
     * @return void
     */
    function gddysec_get_user_lastlogin()
    {
        if (current_user_can('manage_options')
            && GddysecRequest::get(':lastlogin', '1')
        ) {
            $current_user = wp_get_current_user();
            $last_logins = gddysec_get_logins(2, 0, $current_user->ID);
            ...
            if ($last_logins
                ...
                $page_url = GddysecTemplate::getUrl('lastlogins');
                $message = sprintf(
                    'Last login was at <b>%s</b> from <b>%s</b> <em>(%s)</em> '
                    . '<a href="%s" target="_self">view all logs</a>',
                    Gddysec::datetime($row['user_lastlogin_timestamp']),
                    Gddysec::escape($row['user_remoteaddr']),
                    Gddysec::escape($row['user_hostname']),
                    Gddysec::escape($page_url)
                );
    ...
    add_action('network_admin_notices', 'gddysec_get_user_lastlogin');
    add_action('admin_notices', 'gddysec_get_user_lastlogin');
}
