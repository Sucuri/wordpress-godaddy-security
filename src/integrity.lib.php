...
class GddysecIntegrity
{
    ...
    public static function pageIntegrity()
    {
        ...
        return GddysecTemplate::getSection('integrity', $params);
    }
    ...
    {
        if (GddysecRequest::post('form_action') !== 'check_wordpress_integrity') {
            return;
        }
    ...
    private static function pageIntegritySubmission()
    {
        ...
        if ($action === false || !GddysecInterface::checkNonce()) {
        ...
        if (GddysecRequest::post(':process_form') != 1) {
            return GddysecInterface::error('You need to confirm that you understand the risk of this operation.');
        }

        /* skip if the requested action is not currently supported */
        if ($action !== 'fixed' && $action !== 'delete' && $action !== 'restore') {
            return GddysecInterface::error('Requested action is not supported.');
        }

        /* process the HTTP request */
        $cache = new GddysecCache('integrity');
        $core_files = GddysecRequest::post(':integrity', '_array');
        ...
        if (!$core_files) {
            return GddysecInterface::error('Nothing was selected from the list.');
        }

        /* process files until the maximum execution time is reached */
        $startTime = microtime(true);
        $displayTimeoutAlert = false;
        $maxtime = (int) Gddysec::iniGet('max_execution_time');
        ...
        foreach ((array) $core_files as $file_meta) {
            ...
            if ($action === 'restore' && ($status_type === 'removed' || $status_type === 'modified')) {
                $content = GddysecAPI::getOriginalCoreFile($file_path);
        ...
        if (!empty($files_affected)) {
            ...
            switch ($action) {
                ...
                case 'delete':
                    GddysecEvent::reportNoticeEvent($message);
                    break;

                case 'fixed':
                    GddysecEvent::reportWarningEvent($message);
                    break;
            }
        }

        if ($displayTimeoutAlert) {
            GddysecInterface::error('Server is not fast enough to process this action; maximum execution time reached');
        }

        if ($files_processed != $files_selected) {
            return GddysecInterface::error(
                sprintf(
    ...
    public static function getIntegrityStatus($send_email = false)
    {
        ...
        $params['Version'] = Gddysec::siteVersion();
        ...
        $params['Integrity.RemoteChecksumsURL'] = GddysecAPI::checksumAPI();
        ...
        if ($latest_hashes) {
            ...
            foreach ($latest_hashes as $list_type => $file_list) {
                ...
                foreach ($file_list as $file_info) {
                    ...
                    if ($file_size !== false) {
                        $file_size_human = Gddysec::humanFileSize($file_size);
                    }

                    $modified_at = $file_info['modified_at'] ? Gddysec::datetime($file_info['modified_at']) : '';

                    // Generate the HTML code from the snippet template for this file.
                    $params['Integrity.List'] .= GddysecTemplate::getSnippet(
                        'integrity-incorrect',
                        array(
        ...
        if ($send_email === true) {
            if ($affected_files > 0) {
                return GddysecEvent::notifyEvent(
                    'scan_checksums', /* send alert with a list of affected files */
                    GddysecTemplate::getSection('integrity-notification', $params)
                );
            }
        ...
        ob_start();
        $details = GddysecSiteCheck::details();
        $errors = ob_get_clean(); /* capture possible errors */
        $params['SiteCheck.Details'] = empty($errors) ? $details : '<br>'.$errors;

        $params['Integrity.DiffUtility'] = GddysecIntegrity::diffUtility();

        $template = ($affected_files === 0) ? 'correct' : 'incorrect';
        return GddysecTemplate::getSection('integrity-' . $template, $params);
    }
    ...
    public static function diffUtility()
    {
        ...
        $params['DiffUtility.Modal'] = GddysecTemplate::getModal(
            'none',
            array(
        ...
        return GddysecTemplate::getSection('integrity-diff-utility', $params);
    }
    ...
    {
        if (GddysecRequest::post('form_action') !== 'integrity_diff_utility') {
            return;
        }

        ob_start();
        $filename = GddysecRequest::post('filepath');
        echo GddysecCommand::diffHTML($filename);
    ...
    {
        $base_content_dir = '';
        $latest_hashes = GddysecAPI::getOfficialChecksums();
        ...
        $checksumAlgorithm = GddysecAPI::checksumAlgorithm();
        ...
        foreach ($latest_hashes as $file_path => $remote) {
            ...
            if (file_exists($full_filepath)) {
                ...
                $local = GddysecAPI::checksum($checksumAlgorithm, $full_filepath);
