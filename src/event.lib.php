...
class GddysecEvent extends Gddysec
{
    ...
    {
        if (!GddysecAPI::getPluginKey()) {
            return self::throwException('API key is not available');
        }

        $wp_version = self::siteVersion();
        $reported_version = GddysecOption::getOption(':site_version');
        ...
        GddysecEvent::reportInfoEvent('WordPress version detected ' . $wp_version);

        return GddysecOption::updateOption(':site_version', $wp_version);
    }
    ...
    {
        if ($force_scan) {
            return GddysecOption::updateOption(':runtime', time());
        }

        $current_time = time();
        $runtime = (int) GddysecOption::getOption(':runtime');
        $diff = abs($current_time - $runtime);

        if ($diff < GDDYSEC_SCANNER_FREQUENCY) {
            return false;
        }

        return GddysecOption::updateOption(':runtime', $current_time);
    }
    ...
    public static function filesystemScan($force_scan = false)
    {
        ...
        GddysecOption::updateOption(':runtime', time());

        return GddysecAPI::sendHashes($signatures);
    }
    ...
    private static function sendLogToAPI($message = '', $timestamp = '', $timeout = 1)
    {
        ...
        $resp = GddysecAPI::apiCallWordpress('POST', $params, true, $args);
    ...
    public static function sendLogToQueue($message = '')
    {
        ...
        GddysecInterface::createStorageFolder();
         ...
         */
        if (function_exists('gddysec_selfhosting_fpath')) {
            $monitor_fpath = gddysec_selfhosting_fpath();

            if ($monitor_fpath !== false) {
                $local_event = sprintf(
                    "%s WordPressAudit %s %s : %s\n",
                    Gddysec::datetime(null, 'Y-m-d H:i:s'),
                    Gddysec::getTopLevelDomain(),
                    GddysecOption::getOption(':account'),
                    $message
                );
                @file_put_contents(
                    $monitor_fpath,
         ...
         */
        if (GddysecOption::isEnabled(':api_service')) {
    ...
    {
        $email_params = array();
        $email = GddysecOption::getOption(':notify_to');
        $notify = GddysecOption::getOption(':notify_' . $event);
        ...
        switch ($event) {
            ...
            case 'failed_login':
                ...
                $content .= "\n" . sprintf(
                    ...
                    . "='%s'>%s</a> <br>\n[3] <a href='https://www.godaddy.com"
                    . "/help/set-up-my-web-application-firewall-waf-and-cdn-26"
                    . "813'>https://www.godaddy.com/help/set-up-my-web-applica"
                    . "tion-firewall-waf-and-cdn-26813</a><br>\n",
                    $settings_url,
                    $settings_url
                );
                break;
    ...
    {
        if (!$addr) {
            $addr = Gddysec::getRemoteAddr();
        }

        $cache = new GddysecCache('trustip', false);
    ...
    public static function setNewPassword($user_id = 0)
    {
        ...
        $website = Gddysec::getDomain();
        $user_login = $user->user_login;
        $display_name = $user->display_name;
        $new_password = wp_generate_password(15, true, false);

        $message = GddysecTemplate::getSection(
    ...
    public static function setNewConfigKeys()
    {
        ...
        $content = GddysecFileInfo::fileContent($config_path);
        $config_lines = explode("\n", $content); /* maintain new lines */
        $new_keys = GddysecAPI::getNewSecretKeys();
        $new_keys_string = '';
        $old_keys_string = '';
        $old_keys = array();
