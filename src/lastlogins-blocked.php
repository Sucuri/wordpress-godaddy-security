 ...
 * @link       https://wordpress.org/plugins/godaddy-security
 */
class GddysecBlockedUsers extends GddysecLastLogins
{
    /**
    ...
    public static function page()
    {
        ...
        if (GddysecInterface::checkNonce()) {
            $unblockUsers = GddysecRequest::post(':unblock_user', '_array');

            if (is_array($unblockUsers) && !empty($unblockUsers)) {
                self::unblock($unblockUsers);
                GddysecInterface::info('Selected user accounts were unblocked');
            }
        }

        $cache = new GddysecCache('blockedusers', false);
        $blocked = $cache->getAll();

        if (is_array($blocked) && !empty($blocked)) {
            foreach ($blocked as $data) {
                $output['BlockedUsers.List'] .= GddysecTemplate::getSnippet(
        ...
        return GddysecTemplate::getSection('lastlogins-blockedusers', $output);
    }
    ...
    public static function block($users = array())
    {
        if (is_array($users) && !empty($users)) {
            $logs = gddysec_get_all_failed_logins();
            $cache = new GddysecCache('blockedusers');
            $blocked = $cache->getAll();
    ...
    {
        if (!class_exists('GddysecRequest') || !class_exists('GddysecCache')) {
            return;
        }

        $username = GddysecRequest::post('log');
