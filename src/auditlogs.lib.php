...
class GddysecAuditLogs
{
    ...
    public static function pageAuditLogs()
    {
        ...
        $params['AuditLogs.Lifetime'] = GDDYSEC_AUDITLOGS_LIFETIME;

        return GddysecTemplate::getSection('auditlogs', $params);
    }
    ...
    public static function ajaxAuditLogs()
    {
        ...
        $maxPerPage = GDDYSEC_AUDITLOGS_PER_PAGE;
        $pageNumber = GddysecTemplate::pageNumber();
        ...
        if (!$auditlogs || $pageNumber !== 1) {
            ...
            $auditlogs = GddysecAPI::getAuditLogs($logsLimit);
        ...
        $queuelogs = GddysecAPI::getAuditLogsFromQueue();
        ...
        for ($i = $iterator_start; $i < $total_items; $i++) {
            ...
            $snippet_data = array(
                ...
                'AuditLog.Time' => Gddysec::datetime($audit_log['timestamp'], 'H:i'),
                'AuditLog.Date' => Gddysec::datetime($audit_log['timestamp'], 'M d, Y'),
            ...
            if (!empty($snippet_data['AuditLog.Date'])) {
                $snippet_data['AuditLog.Date'] = '<div class="gddysec-aud'
            ...
            if ($audit_log['file_list']) {
                $snippet_data['AuditLog.Extra'] .= '<ul class="gddysec-list-as-table">';
            ...
            $response['content'] .= GddysecTemplate::getSnippet('auditlogs', $snippet_data);
            $counter_i += 1;
        }
        ...
        if ($total_items > 1) {
            ...
            if ($maxpages > GDDYSEC_MAX_PAGINATION_BUTTONS) {
                $maxpages = GDDYSEC_MAX_PAGINATION_BUTTONS;
            }
    ...
    {
        if (GddysecRequest::post('form_action') !== 'auditlogs_send_logs') {
            return;
        }

        /* blocking; might take a while */
        wp_send_json(GddysecEvent::sendLogsFromQueue(), 200);
    }
