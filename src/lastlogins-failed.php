 ...
 */
function gddysec_failed_logins_panel()
{
    ...
    if ($failed_logins) {
        ...
        $template_variables['FailedLogins.PaginationLinks'] = GddysecTemplate::pagination(
            '%%GDDYSEC.URL.Lastlogins%%#failed',
            $failed_logins['count'],
            $max_per_page
        );
    ...
    return GddysecTemplate::getSection('lastlogins-failedlogins', $template_variables);
}
 ...
 *
 * @see gddysec_reset_failed_logins()
 *
 ...
 */
function gddysec_failed_logins_datastore_path($get_old_logs = false, $reset = false)
{
    $file_name = $get_old_logs ? 'gddysec-oldfailedlogins.php' : 'gddysec-failedlogins.php';
    $datastore_path = Gddysec::dataStorePath($file_name);
    $default_content = gddysec_failed_logins_default_content();
 ...
 */
function gddysec_failed_logins_default_content()
{
    return "<?php exit(0); ?>\n";
}
 ...
 */
function gddysec_get_all_failed_logins($offset = 0, $limit = -1)
{
    $all = array();
    $new = gddysec_get_failed_logins();
    $old = gddysec_get_failed_logins(true, $offset, $limit);
 ...
 */
function gddysec_get_failed_logins($get_old_logs = false, $offset = 0, $limit = -1)
{
    $datastore_path = gddysec_failed_logins_datastore_path($get_old_logs);

    if (!$datastore_path) {
        return false;
    }

    $lines = GddysecFileInfo::fileLines($datastore_path);
 ...
 */
function gddysec_report_failed_logins($failed_logins = array())
{
    if (!$failed_logins
    ...
    $mail_content = '';
    $prettify_mails = GddysecMail::prettifyMails();
    ...
    foreach ($failed_logins['entries'] as $login_data) {
        $login_data['attempt_date'] = Gddysec::datetime($login_data['attempt_time']);
    ...
    if (GddysecEvent::notifyEvent('bruteforce_attack', $mail_content)) {
        gddysec_reset_failed_logins();
        return true;
    }
 ...
 */
function gddysec_reset_failed_logins()
{
    $datastore_path = Gddysec::dataStorePath('gddysec-failedlogins.php');
    $datastore_backup_path = gddysec_failed_logins_datastore_path(true, false);
    $default_content = gddysec_failed_logins_default_content();
    ...
    return (bool) gddysec_failed_logins_datastore_path(false, true);
}
