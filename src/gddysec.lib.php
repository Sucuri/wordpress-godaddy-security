...
class Gddysec
{
    ...
    public static function siteVersion()
    {
        ...
        $lines = GddysecFileInfo::fileLines($filename);
    ...
    {
        GddysecEvent::filesystemScan();
        GddysecEvent::reportSiteVersion();
        GddysecIntegrity::getIntegrityStatus(true);
        GddysecSettingsPosthack::availableUpdatesContent(true);
        GddysecEvent::sendLogsFromQueue(); /* blocking; keep at the end */
    }
    ...
    private static function orderedHttpHeaders()
    {
        ...
        $addr_header = GddysecOption::getOption(':addr_header');
        $ordered[] = $addr_header;
    ...
    public static function datetime($timestamp = null, $format = null)
    {
        ...
        $tz = GddysecOption::getOption(':timezone');
        ...
        if ($length === 9
            ...
            GddysecOption::deleteOption(':timezone');
        }

        if ($format === null) {
            $format = sprintf(
                "%s\x20%s",
                GddysecOption::getOption('date_format'),
                GddysecOption::getOption('time_format')
            );
        }
    ...
    {
        return substr(md5_file(GDDYSEC_PLUGIN_PATH . '/' . $file), 0, 7);
    }
}
